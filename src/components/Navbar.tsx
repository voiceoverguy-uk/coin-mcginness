    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-cinema-darker/95 backdrop-blur-md border-b border-cinema-border'
            : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            <Link href="/" className="flex-shrink-0">
              ...
              <span className="hidden sm:inline text-sm text-cinema-muted ml-2 font-light tracking-widest uppercase">
                Composer
              </span>
            </Link>
      ...
      {/* Slide-in panel */}
      <div
        className={`fixed top-0 right-0 h-full w-64 z-50 md:hidden bg-cinema-darker/98 backdrop-blur-md border-l border-cinema-border shadow-2xl transition-transform duration-300 ease-in-out ${
        ...
        <div className="pt-20 px-6 space-y-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`block py-3 text-base font-medium tracking-wide transition-colors border-b border-cinema-border/40 ${
                pathname === link.href
                  ? 'text-white'
